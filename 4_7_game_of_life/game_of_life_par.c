// Compile: gcc -O3 game_of_life_par.c -o game_of_life_par.out -fopenmp
// Execute: ./game_of_life_par.out <num_threads>
#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
...
void findNextGen(int mat[M][N]) {
    ...
    // Parallelize the outer loop (loop over rows)
    ...
    // Copy the next generation matrix back to the original matrix
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            mat[i][j] = nextGen[i][j];
        }
